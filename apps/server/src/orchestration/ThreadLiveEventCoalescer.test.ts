...
const encodeEvent = Schema.encodeSync(Schema.fromJsonString(OrchestrationThreadDetailEvent));
...
function makeMessage(sequence: number, text = "Still working"): OrchestrationThreadDetailEvent {
  return {
    sequence,
    eventId: EventId.make(`event-${sequence}`),
    aggregateKind: "thread",
