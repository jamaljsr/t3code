...
export type ThreadLiveInput =
  | { readonly kind: "event"; readonly event: OrchestrationThreadDetailEvent }
  | { readonly kind: "synchronized" };

function isToolUpdated(event: OrchestrationThreadDetailEvent): boolean {
  return (
...
function stableToolCallIdentity(event: OrchestrationThreadDetailEvent): string | null {
...
export function coalesceLiveToolUpdatedEvents(
  events: ReadonlyArray<OrchestrationThreadDetailEvent>,
): ReadonlyArray<OrchestrationThreadDetailEvent> {
  const survivors: Array<OrchestrationThreadDetailEvent> = [];
  let pendingUpdates: Array<OrchestrationThreadDetailEvent> = [];

  const flushUpdates = () => {
    const seen = new Set<string>();
    const latestUpdates: Array<OrchestrationThreadDetailEvent> = [];
...
export const makeThreadLiveEventCoalescer = Effect.fn("makeThreadLiveEventCoalescer")(
  function* (options?: {
    ...
    let pendingUpdates: Array<RetainedLiveItem<OrchestrationThreadDetailEvent>> = [];
