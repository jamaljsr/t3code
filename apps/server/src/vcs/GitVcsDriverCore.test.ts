...
it.layer(TestLayer)("GitVcsDriver core integration", (it) => {
  ...
  describe("review diff previews", () => {
    ...
    it.effect("caps the combined worktree patch and marks it truncated", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTmpDir();
        yield* initRepoWithCommit(cwd);
        const driver = yield* GitVcsDriver.GitVcsDriver;
        ...
        assert.strictEqual(workingTree?.files.length, 2);
        assert.strictEqual(workingTree?.truncated, true);
        assert.isTrue(workingTree?.diff.endsWith("\n\n[truncated]"));
        assert.isAtMost(
          new TextEncoder().encode(workingTree?.diff ?? "").byteLength,
          120_000 + new TextEncoder().encode("\n\n[truncated]").byteLength,
        );
      }),
    );

    it.effect("marks a capped branch patch with the truncation marker", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTmpDir();
        const { initialBranch } = yield* initRepoWithCommit(cwd);
        const driver = yield* GitVcsDriver.GitVcsDriver;
        yield* git(cwd, ["checkout", "-b", "feature/large-diff"]);
        yield* writeTextFile(cwd, "large.txt", `${"x".repeat(140_000)}\n`);
        ...
        assert.strictEqual(branchRange?.truncated, true);
        assert.isTrue(branchRange?.diff.endsWith("\n\n[truncated]"));
        assert.isAtMost(
          new TextEncoder().encode(branchRange?.diff ?? "").byteLength,
          120_000 + new TextEncoder().encode("\n\n[truncated]").byteLength,
        );
      }),
    );
      ...
      Effect.gen(function* () {
        ...
        for (const source of preview.sources) {
          for (const file of source.files) {
            const patch = yield* driver.getReviewDiffFilePatch({
              cwd,
