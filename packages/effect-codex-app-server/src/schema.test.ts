...
it("decodes user-input requests from before the blocking flag was added", () => {
  const request = {
    itemId: "item-1",
    threadId: "thread-1",
    turnId: "turn-1",
  ...
  for (const schema of [
    ...
    assert.equal(Schema.is(schema)(request), true);
    assert.equal(Schema.is(schema)({ ...request, isBlocking: true }), true);
  }
});
